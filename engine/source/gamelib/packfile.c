...
int openPackfile(const char *filename, const char *packfilename)
{
    ...
    if((handle = open(filename, O_RDONLY | O_BINARY | O_CREAT, 777)) != -1)
    {
    ...
    if (fspath != NULL)
    {
        if((handle = open(fspath, O_RDONLY | O_BINARY | O_CREAT, 777)) != -1)
        {
            if((packfilesize[h] = lseek(handle, 0, SEEK_END)) == -1)
            {
#ifdef VERBOSE
    ...
    if((handle = open(packfilename, O_RDONLY | O_BINARY | O_CREAT, 777)) == -1)
    {
#ifdef VERBOSE
        printf ("perm err\n");
#endif
...
int pak_init()
{
    ...
    else
    {
        ...
        pakfd = open(packfile, O_RDONLY | O_BINARY | O_CREAT, 777);
